...
#SBATCH --gpus=1
#SBATCH --job-name=ECGFTmed
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --time=2:00:00
#SBATCH --array=0-5%6
#SBATCH --output=output/fine_tune_ecg/unbalanced/medium2D/%A_%a.out
...
for split in "${splits[@]}"; do
    ...
    if [ "${split}" == "train" ]; then
        ...
        VAL_LEADS="${UNBAL_LEADS_PTH}"
        VAL_LABELS="${UNBAL_LABELS_PTH}"
    else  # test
        TEST_LEADS="${UNBAL_LEADS_PTH}"
        TEST_LABELS="${UNBAL_LABELS_PTH}"
    fi
done
