...
# TODO: gloobal pooling & projection
# TODO: eliminate clip loss outside in the training loop
class ClipContrastiveLearning(torch.nn.Module, ABC):
    def __init__(self, input_size:int=2, return_latent_space:bool=False,
        ...
        self.cmr_encoder = get_model(pretrained=self.pretrained,
            return_latent_space=True, contrastive_learning=True, args=args)

        self.ecg_encoder = ... # find a way to call the MAE ECG Enocder
        ...
        # TODO implement clip loss
        self.clip_loss = ...

    def forward(self, x_ecg, x_cmr):

        # should return the latent representations
        ecg_features = self.ecg_encoder(x_ecg) 
        cmr_features = self.cmr_encoder(x_cmr)

        return ecg_features, cmr_features
...
# TODO properly import the models from CMR, instead of copy/pasting from it
...
def get_model(model_name, input_size, latent_dim, num_outputs, args):
